...
directories = [datapath+"\\cucumbers"]
labels_csv_path =datapath+'\\cucumbers_fitness.csv'
local_path = os.path.dirname(__file__)  # Define local_path if it's not already defined
csv_file_path = os.path.join(local_path, 'cucumbers_result.csv')
...
class CucumberDataset(Dataset):
    def __init__(self, image_files, labels_df,transform=None):
        ...
        self.labels_df = labels_df.dropna().copy()
        self.labels_df.loc[:, 'A'] = self.labels_df['A'].astype(int)
        self.labels_df.loc[:, 'B'] = self.labels_df['B'].astype(int)
        self.labels_df.loc[:, 'folder'] = self.labels_df['folder'].astype(str)
    ...
    def __getitem__(self, idx):
        ...
        try:
            label_value = self.labels_df.loc[(self.labels_df['folder'] == folder) & (self.labels_df['A'] == int(group))& (self.labels_df['B'] == int(cucumber_number)), 'fitness'].values[0]   
            #print("label_value: ",label_value)
            if 0<= label_value <= 25:
                bin_label=int(label_value/3)
...
all_train_dataset, test_dataset = train_test_split(dataset2, test_size=187,stratify=spl2, random_state=1)
...
firs_time=0
curent_loop=0
all_split_sizes=[ 31,  47,  63,  79,  95, 111, 127, 143]
